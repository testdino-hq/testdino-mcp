...
7. **`get_audit_report`** - Read side + start of the Playwright test audit flow. `action="context"` fetches the server-curated audit prompt and branch signals to START an audit; `action="list"` browses previously submitted reports (optional `branch` filter); `action="get"` retrieves one saved report by `reportId` and can save `TEST-AUDIT.md` locally.
8. **`submit_audit_report`** - Write side of the audit flow — the FINAL step. Persists a completed audit report (requires `orgId` + `projectId` + `score`; plus findings, recommendations, markdown via `markdownReportPath`) back to TestDino, and optionally saves `TEST-AUDIT.md` locally. Resolve `orgId` via `health()` if you don't have it.
