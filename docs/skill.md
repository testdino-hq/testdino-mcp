...
The audit flow is run with two tools — `get_audit_report` (read side + start) and `submit_audit_report` (write side + final step). It is triggered by a **named request only**. There are exactly two scenarios:
...
**This applies even when the target is Playwright test code.** A generic "test audit" on a Playwright file is still just a normal audit conversation — the tools are not triggered unless the user explicitly names TestDino.
...
#### Scenario 2 — Explicit TestDino audit request → call `get_audit_report` then `submit_audit_report` (Playwright only)
...
**Doing any of the above without calling the tool is a protocol violation** — you would be claiming to produce a TestDino audit without actually running TestDino's audit flow. TestDino audits require the server-curated prompt and branch signals returned by `get_audit_report(action="context")`; you do not have those without the tool call. "TestDino Test Audit" is the name of a capability that runs via these tools, not a style of output you can imitate.
...
**When both conditions hold (explicit TestDino + Playwright target), run the full three-step flow**:

1. `get_audit_report(action="context", projectId=..., branch="...")` — IMMEDIATE first action. This fetches the server-curated prompt and branch signals to START the audit. Use `health()` first if you don't have a `projectId` yet.
...
3. `submit_audit_report(orgId=..., projectId=..., markdownReportPath="TEST-AUDIT.md", score=..., findings=[...], recommendations=[...], reportName=...)` — the FINAL step that persists the completed report to TestDino. `orgId` is required; resolve it via `health()` if you don't have it.

`get_audit_report(action="list")` browses past reports; `get_audit_report(action="get", reportId="...")` retrieves one saved report.
...
- If `get_audit_report` or `submit_audit_report` returns `PROJECT_NOT_FOUND`, auth, or access errors, stop and resolve `orgId`/`projectId` with `health()` before continuing. Do not generate a pseudo-audit from local files alone.
...
**Purpose**: The READ side and the START of the Playwright test-quality audit flow. Fetch the server-curated audit prompt + branch signals to begin an audit (`action="context"`), browse previously submitted reports (`action="list"`), or retrieve one saved report (`action="get"`). Read-only (`readOnlyHint: true`). Only triggered when the user explicitly names TestDino.
...
| Action | When to use |
|--------|-------------|
| `context` | Fetch the server-curated audit prompt + branch signals to START an audit (this is the IMMEDIATE first action when triggered) |
| `list` | Browse previously submitted reports for the project |
| `get` | Retrieve a specific saved audit report by `reportId` |

**Parameters**:
| Parameter | Type | Description |
|-----------|------|-------------|
| `branch` | string | For `context`: the branch to audit (optional, auto-detected from git). For `list`: optional branch filter |
| `reportId` | string | Required for `get` |
| `limit` | number | Page size for `list` |
| `page` | number | Page number for `list` |
| `writeMarkdown` | boolean | For `get`: write the retrieved markdown report to a local file |
| `outputPath` | string | For `get`: relative destination for the local markdown file. Relative paths resolve from `TESTDINO_MCP_WORKSPACE` when set, otherwise from the MCP process cwd. Defaults to `TEST-AUDIT.md` |

**How to use it well**:

1. Start with `get_audit_report(action="context", branch="...")` to fetch the prompt, branch signals, and prior audit findings. This is the IMMEDIATE first action when a TestDino audit is triggered.
2. If the user named a feature/spec area, keep the local analysis (after this call returns) centered on that slice plus its shared helpers/setup. Pass the smallest correct `scope` plus explicit `target` to `submit_audit_report` later.
3. If `context` fails because the project is missing or access is denied, call `health()` and resolve the right `projectId` before continuing. Do not write a fallback local-only audit and present it as a TestDino audit.
4. After analyzing locally and submitting via `submit_audit_report`, use `get_audit_report(action="list")` to browse all saved reports — add `branch="..."` for branch-specific history — and `get_audit_report(action="get", reportId="...", writeMarkdown=true)` to save `TEST-AUDIT.md` locally when needed.
...
→ (analyze locally, write TEST-AUDIT.md, then call submit_audit_report — see below)
...
**Purpose**: The WRITE side and the FINAL STEP of the audit flow — persist a completed Playwright audit report back to TestDino. Not read-only (`readOnlyHint: false`, `destructiveHint: false`). Call this only after `get_audit_report(action="context")` returned and you analyzed the local code and wrote a markdown report.

**Required parameters**: `orgId`, `projectId`, `score`

**Parameters**:
| Parameter | Type | Description |
|-----------|------|-------------|
| `orgId` | string | Organization ID. REQUIRED — resolve via `health()` if you don't have it |
| `score` | number | Final audit score, 0–100. Required |
| `branch` | string | Branch the report covers (optional, auto-detected from git) |
| `scope` | string | `'testcase'`, `'feature'`, `'spec_file'`, or `'suite'`. Defaults to `'suite'` |
...
| `findings` | array | Structured findings — each: `title`, `category`, `severity` (`critical`/`high`/`medium`/`low`), `summary`, `recommendation`, `evidence` (`file`, `lineStart`, `lineEnd`, `observation`); optional `subCategory`. `category` must be one of the audit category codes |
| `recommendations` | array | Recommendation strings for the completed report |
| `markdownReport` | string | Inline completed markdown report. Prefer `markdownReportPath` |
| `markdownReportPath` | string | PREFERRED — relative (from `TESTDINO_MCP_WORKSPACE`) or absolute path to a local markdown file to read for submission |
| `writeMarkdown` | boolean | Also write the submitted report locally (skipped when read from `markdownReportPath`) |
| `outputPath` | string | Optional relative destination for the local markdown file. Defaults to `TEST-AUDIT.md` |

**How to use it well**:

1. Convert the audit into `score`, `findings`, `recommendations`, choose a short `reportName`, and write the markdown report to a local file such as `TEST-AUDIT.md`.
2. Submit with `submit_audit_report(orgId=..., branch="...", reportName="...", score=..., markdownReportPath="TEST-AUDIT.md")`. `orgId` is required — resolve it via `health()` if you don't already have it. Set `TESTDINO_MCP_WORKSPACE` to your repo root if the MCP starts outside the project, or pass an absolute path. Use inline `markdownReport` only when a local file is not practical.
...
4. If submission fails with `PROJECT_NOT_FOUND`, auth, or access errors, call `health()` and resolve the right `orgId`/`projectId` before retrying. Do not present a local-only fallback as a TestDino audit.

**Pattern**:

```
(after get_audit_report(action="context", ...) returned and TEST-AUDIT.md was written)
submit_audit_report(orgId="<from health()>", branch="main", reportName="Login Flow Tests", score=88, scope="feature", target={ featureName: "Auth / Login" }, findings=[...], recommendations=[...], markdownReportPath="TEST-AUDIT.md", writeMarkdown=true)
→ Report persisted to TestDino
→ Optionally: get_audit_report(action="list") or get_audit_report(action="get", reportId="<id>")
```
...
| `get_audit_report`         | `projectId`, `action` (+ `reportId` for `get`)                                                                                   |
| `submit_audit_report`      | `orgId`, `projectId`, `score` (+ markdown via `markdownReportPath`/`markdownReport`)                                             |
