...
| `by_artifacts`      | boolean | No       | Filter by artifact presence. `true` keeps only cases with a downloadable artifact (screenshot/video/trace) on ANY attempt — including a trace from a flaky test's earlier failed attempt; `false` keeps only cases with none. Unavailable/failed-upload attachments do not count (default: false). |
