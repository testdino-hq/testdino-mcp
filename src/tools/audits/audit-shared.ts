/**
 * Shared helpers for the TestDino Playwright audit tools.
 *
 * The audit feature is exposed as two tools — `get_audit_report` (read-only)
 * and `submit_audit_report` (write) — so that no single tool mixes safe and
 * unsafe operations (Anthropic Connectors Directory requirement). Both tools
 * share the same local-file handling, branch detection, markdown normalization,
 * and findings sanitization, which lives here.
 */
...
/** Comma-separated list of valid audit category codes, for tool descriptions. */
...
export function assertWithinWorkspaceBase(
  fieldName: string,
