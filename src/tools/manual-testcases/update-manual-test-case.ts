...
export const updateManualTestCaseTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            updates: {
                type: "object",
                description: "Object containing the fields to update. Can include: title, description, steps, status, priority, severity, type, layer, behavior, preconditions, postconditions, etc.",
                properties: {
                    title: {
                        type: "string",
...
export async function handleUpdateManualTestCase(args: any) {
    ...
    try {
        ...
        const updateManualTestCaseUrl = endpoints.updateManualTestCase(String(args.projectId), String(args.caseId));

        const response = await apiRequestJson(updateManualTestCaseUrl, {
            method: "PATCH",
            headers: {
        ...
        throw new Error(
            `Failed to update manual test case: ${error?.message || String(error)}`
        );
    }
}
