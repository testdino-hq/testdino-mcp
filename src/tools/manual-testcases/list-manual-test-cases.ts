...
export const listManualTestCasesTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            search: {
                type: "string",
                description: "Search term to match against title, description, or caseId. Example: 'login' or 'TC-123'.",
            },
            suiteId: {
                type: "string",
                description: "Filter by specific test suite ID. Use list_manual_test_suites to find suite IDs.",
            },
            status: {
                type: "string",
            ...
            tags: {
                type: "string",
                description: "Filter by tags (comma-separated list). Example: 'smoke,regression' or 'critical'.",
            },
            isFlaky: {
                type: "boolean",
...
export async function handleListManualTestCases(args: any) {
    ...
    try {
        ...
        const response = await apiRequestJson(listManualTestCasesUrl, {
            headers: {
        ...
        throw new Error(
            `Failed to list manual test cases: ${error?.message || String(error)}`
        );
    }
}
